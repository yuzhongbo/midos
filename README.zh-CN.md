...
- Semantic memory anti-pollution controls (optional, JVM properties):
  ...
  - optional ONNX embedding runtime: `mindos.memory.embedding.onnx.enabled`, `mindos.memory.embedding.onnx.preset` (`custom|bge-micro`, default `custom`, preset will auto-fill BGE-Micro paths/dimensions if unset), `mindos.memory.embedding.onnx.model-path`, `mindos.memory.embedding.onnx.tokenizer-path`, `mindos.memory.embedding.onnx.output-name`, `mindos.memory.embedding.onnx.dimensions`, `mindos.memory.embedding.onnx.batch-size`, `mindos.memory.embedding.onnx.max-length`, `mindos.memory.embedding.onnx.intra-op-threads`, `mindos.memory.embedding.onnx.inter-op-threads`, `mindos.memory.embedding.onnx.cache.*`
