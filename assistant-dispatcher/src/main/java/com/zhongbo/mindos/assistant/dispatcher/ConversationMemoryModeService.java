...
final class ConversationMemoryModeService {
    ...
    boolean isExplicitMemoryRecallRequest(String userInput) {
        ...
        return normalized.contains("根据记忆")
                ...
                || normalized.contains("刚才说啥")
                || normalized.contains("刚刚说啥")
                || normalized.contains("刚才讲啥")
                || normalized.contains("刚刚讲啥")
                || normalized.contains("前面说啥")
                || normalized.contains("上面说啥")
                || normalized.contains("刚才聊到哪")
                || normalized.contains("刚刚聊到哪")
                || normalized.contains("我们刚才聊了什么")
                || normalized.contains("你刚才说的")
                || normalized.contains("你刚刚说的")
                || normalized.contains("刚才提到的")
                || normalized.contains("刚刚提到的")
