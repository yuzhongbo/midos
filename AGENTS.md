...
- IM integration flow (optional): `/api/im/feishu/events`, `/api/im/dingtalk/events`, `/api/im/wechat/events` -> dispatcher chat route -> platform text response; DingTalk can also acknowledge immediately and push the final reply later through event `sessionWebhook` when `mindos.im.dingtalk.async-reply.*` is enabled (`allow-insecure-localhost-http` should stay local-test only).
