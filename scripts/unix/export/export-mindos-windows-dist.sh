...
# Generic MCP tools (non-search only). If the same alias appears in both maps,
# explicit MCP server entries win over MINDOS_SKILLS_SEARCH_SOURCES.
# MINDOS_SKILLS_MCP_SERVERS=
# MINDOS_SKILLS_MCP_SERVER_HEADERS=

# Search-source examples:
# MINDOS_SKILLS_SEARCH_SOURCES=serper:https://google.serper.dev/search;news-url=https://google.serper.dev/news;api-key=xxxx
# MINDOS_SKILLS_SEARCH_SOURCES=serper:https://google.serper.dev/search;news-url=https://google.serper.dev/news;api-key=xxxx,serpapi:https://serpapi.com/search.json?engine=google_news;api-key=yyyy

# Generic MCP example (non-search only):
